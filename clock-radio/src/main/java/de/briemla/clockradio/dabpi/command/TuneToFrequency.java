...
public class TuneToFrequency extends BaseCommand<TuneToFrequencyResult> {
    ...
    protected TuneToFrequencyResult parseSpecialized(Output output) {
        Optional<TuneToFrequencyResult> tunedFrequency = output.standardAsStream().filter(line -> line.startsWith(FILTER)).map(this::convert).findFirst();
    ...
    private void check(Integer parsedFrequency) {
        if (!frequency.equals(parsedFrequency)) {
            throw new IllegalArgumentException("Tuned frequency differs from expected frequency: tuned: " + parsedFrequency + " expected: " + frequency);
        }
    }
